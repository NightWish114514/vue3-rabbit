...
const spliter = '★'
// 根据skus数据得到路径字典对象
const getPathMap = (skus: DetailSkuType[]): PathMap => {
...
// 初始化禁用状态
function initDisabledStatus (specs: SpecType[], pathMap: PathMap) {
...
// 得到当前选中规格集合
const getSelectedArr = (specs: SpecType[]): (string | undefined)[] => {
...
// 更新按钮的禁用状态
const updateDisabledStatus = (specs: SpecType[], pathMap: PathMap) => {
  // 遍历每一种规格
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const clickSpecs = (item: SpecType, val: SpecValueType) => {
      ...
      if (selectedArr.length === props.goods.specs.length) {
        ...
        if (skuIds && skuIds.length > 0) {
          ...
          const sku = props.goods.skus.find((sku: DetailSkuType) => sku.id === skuId)
          if (sku) {
            // 传递数据给父组件
            emit('change', {
              skuId: sku.id,
              ...
              specsText: sku.specs.reduce((p: string, n: { name: string; valueName: string }) => `${p} ${n.name}：${n.valueName}`, '').trim()
            })
          }
        }
      } else {
